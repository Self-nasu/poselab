...
export const MinecraftCharacter = ({ skinImage, pose }: MinecraftCharacterProps) => {
    ...
    useEffect(() => {
        ...
        const safePose = {
            ...
            leftUpperArm: pose?.leftUpperArm || { x: 0, y: 0, z: 0 },
            leftLowerArm: pose?.leftLowerArm || { x: 0, y: 0, z: 0 },
            rightUpperArm: pose?.rightUpperArm || { x: 0, y: 0, z: 0 },
            rightLowerArm: pose?.rightLowerArm || { x: 0, y: 0, z: 0 },
            leftUpperLeg: pose?.leftUpperLeg || { x: 0, y: 0, z: 0 },
            leftLowerLeg: pose?.leftLowerLeg || { x: 0, y: 0, z: 0 },
            rightUpperLeg: pose?.rightUpperLeg || { x: 0, y: 0, z: 0 },
            rightLowerLeg: pose?.rightLowerLeg || { x: 0, y: 0, z: 0 },
        };

        if (headRef.current) {
            headRef.current.rotation.set(
                (safePose.head.x * Math.PI) / 180,
                (safePose.head.y * Math.PI) / 180,
                (safePose.head.z * Math.PI) / 180
            );
        }
        if (bodyRef.current) {
            bodyRef.current.rotation.set(
                (safePose.body.x * Math.PI) / 180,
                (safePose.body.y * Math.PI) / 180,
                (safePose.body.z * Math.PI) / 180
            );
        }
        ...
        if (leftUpperArmRef.current) {
            leftUpperArmRef.current.rotation.set(
                (safePose.leftUpperArm.x * Math.PI) / 180,
                (safePose.leftUpperArm.y * Math.PI) / 180,
                (safePose.leftUpperArm.z * Math.PI) / 180
            );
        }
        if (leftLowerArmRef.current) {
            leftLowerArmRef.current.rotation.set(
                (safePose.leftLowerArm.x * Math.PI) / 180,
                (safePose.leftLowerArm.y * Math.PI) / 180,
                (safePose.leftLowerArm.z * Math.PI) / 180
            );
        }
        ...
        if (rightUpperArmRef.current) {
            rightUpperArmRef.current.rotation.set(
                (safePose.rightUpperArm.x * Math.PI) / 180,
                (safePose.rightUpperArm.y * Math.PI) / 180,
                (safePose.rightUpperArm.z * Math.PI) / 180
            );
        }
        if (rightLowerArmRef.current) {
            rightLowerArmRef.current.rotation.set(
                (safePose.rightLowerArm.x * Math.PI) / 180,
                (safePose.rightLowerArm.y * Math.PI) / 180,
                (safePose.rightLowerArm.z * Math.PI) / 180
            );
        }
        ...
        if (leftUpperLegRef.current) {
            leftUpperLegRef.current.rotation.set(
                (safePose.leftUpperLeg.x * Math.PI) / 180,
                (safePose.leftUpperLeg.y * Math.PI) / 180,
                (safePose.leftUpperLeg.z * Math.PI) / 180
            );
        }
        if (leftLowerLegRef.current) {
            leftLowerLegRef.current.rotation.set(
                (safePose.leftLowerLeg.x * Math.PI) / 180,
                (safePose.leftLowerLeg.y * Math.PI) / 180,
                (safePose.leftLowerLeg.z * Math.PI) / 180
            );
        }
        ...
        if (rightUpperLegRef.current) {
            rightUpperLegRef.current.rotation.set(
                (safePose.rightUpperLeg.x * Math.PI) / 180,
                (safePose.rightUpperLeg.y * Math.PI) / 180,
                (safePose.rightUpperLeg.z * Math.PI) / 180
            );
        }
        if (rightLowerLegRef.current) {
            rightLowerLegRef.current.rotation.set(
                (safePose.rightLowerLeg.x * Math.PI) / 180,
                (safePose.rightLowerLeg.y * Math.PI) / 180,
                (safePose.rightLowerLeg.z * Math.PI) / 180
            );
        }
    }, [pose]);
