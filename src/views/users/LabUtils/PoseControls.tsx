...
interface PoseControlsProps {
  ...
  onPoseChange: (bodyPart: keyof PoseControlsProps['pose'], axis: 'x' | 'y' | 'z', value: number) => void;
}
                ...
                <div key={axis.key} className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-muted-foreground font-mono">
                      {pose[bodyPart.key][axis.key]}°
                    </span>
                  </div>
                  <Slider
                    value={[pose[bodyPart.key][axis.key]]}
                    onValueChange={([value]) => onPoseChange(bodyPart.key, axis.key, value)}
                    min={-180}
                    max={180}
                    step={5}
